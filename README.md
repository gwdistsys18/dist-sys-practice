...
2. Video: VMs Versus Containers --elapsed time: 15 mins
    ...
    | OS layer          | An operating system that runs on VM has virtual hardware layer.  The total number of drivers is small because permutation is small in VM layer.  What's more, there is some configuration of dependency required for loading and automation purpose. | There is a docker engine embedded in the operating system.  Some complexity could be added here--let us set a Hypervisor and VM between OS and Physical layer.  VM could minimize the amount of driver and technologies for the container host. |
    | VM layer         | It has limited number of network interfaces(NICs), storage interfaces.  It can scale at demand.                                                                                                                                                      | (Optional)VM could minimize amount of dirver and technologies for the container host.                                                                                                                                                           |
    | Hypervisor layer | It should interact with physical hardware, like hardware NIC, hardware storage.  There should be agents that monitor RAID card and hardwares in hypervisor.  Not to mention some kernel modules.                                                     | (Optional)Hypervisor can mask all hardware interface for VM, and container host.                                                                                                                                                                |
    | Physical hardware   | \                                                                                                                                                                                                                                                    | \                                                                                                                                                                                                                                               |
